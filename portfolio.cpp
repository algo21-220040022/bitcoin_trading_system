...
// Constructor
NaivePortfolio::NaivePortfolio(HistoricalDataHandler* i_bars, vector<string>* i_symbol_list, boost::ptr_vector<Event>* i_events, string* i_start_date, string* i_end_date, double* i_initial_capital) {
...
void NaivePortfolio::update_timeindex() {
    //map<string, map<string, pair<long,double>>> lastbar;
    long date;
    double sumvalues = 0;
    ...
    for (int i = 0; i < symbol_list->size(); i++) {
        ...
        // Update positions
        auto close_list = bars->get_latest_ticker(symbol, 1)["close"];
        
        date = close_list.begin()->first;
        ...
        double market_value = current_positions[symbol] * close_list.begin()->second;
...
void NaivePortfolio::update_holdings_from_fill(FillEvent fill) {
    ...
    double fill_cost = bars->get_latest_ticker(fill.symbol, 1)["close"].begin()->second;
...
void NaivePortfolio::generate_naive_order(SignalEvent signal) {
    ...
    double mkt_quantity = calculate_quantity(symbol, strength);
...
double NaivePortfolio::calculate_quantity(string symbol, double percentage) {
    ...
    double sellingprice = bars->get_latest_ticker(symbol, 1)["close"].begin()->second;
    ...
    if (requiredcash <= 100 * sellingprice * (1+0.005)) {
        return requiredcash / (sellingprice * 1.005);
    }
    else {
        return requiredcash / (sellingprice * 1.002);
    }
}
